...
lower_yellow = np.array([20, 50, 100], dtype=np.uint8) # Lower threshold value for detecting yellow color
...
def region_of_interest(edges):
    ...
    # Create a RoI
    polygon = np.array([[
        (0, height * (8 / 8)), # left bottom coord
        (0, height * (5 / 8)), # left top coord
        (width, height * (5 / 8)), # right top coord
        (width, height * (8 / 8)), # right bottom coord
    ]], np.int32)
...
def average_slope_intercept(frame, line_segments):
    ...
    if len(right_fit) > 0 and len(left_fit)>0:
        print(left_fit_average[0], right_fit_average[0])
    """
    return lane_lines
...
bt_cmd = None
port = "COM9"  # Port that is used to send signals to a connected RC car
...
while True:    
    ...
    ############################################################################
        ...
        while True:
            ...
            ##### 1.1 SLD (Perception) #####
            ################################
            red_mask = cv2.inRange(hsv, lower_red, upper_red) + cv2.inRange(hsv, lower_red2, upper_red2) # create a red_mask
            boxes, hierarchy = cv2.findContours(red_mask, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE) # detect stop lines using a red mask

            #################################
            ##### 1.2 LKAS (Perception) #####
            #################################
            edges = detect_edges(hsv) # get yellow edges
            roi = region_of_interest(edges)
            line_segments = detect_line_segments(roi)
            lane_lines = average_slope_intercept(frame, line_segments)
            lane_lines_image = display_lines(frame, lane_lines) # (0.8)*frame + (0.2)*(line image)
            steering_angle = (get_steering_angle(frame, lane_lines))
            heading_image, center_x1, center_y1, center_x2, center_y2 = display_heading_line(lane_lines_image, steering_angle)
            ...
            h = 0
            for idx, box in enumerate(boxes):
                if idx==0: # 0번 index box는 화면 전체를 box로 인식 해서 예외처리 해주기
                    continue
                x, y, w, h = cv2.boundingRect(box)
                if y > 500 :  # stop line이기 위한 조건 -> w > 100 and h < 50
                    cv2.rectangle(heading_image, (x, y), (x + w, y + h), (255, 0, 0), 5)
                    cv2.putText(heading_image, 'Stop Line', (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                    if y > 620: # 정지하기 위한 조건
                        flag_StopLine = True
                        print('detected sl')
                        break # 빨리 for문 빠져나가기
            ...
            if lane_lines is not None: # If lines are detected
            ...
            if lkas_iterate_count == iterate_num: # Check if iterate count has reached iterate num
                if threshold_left:
                    if threshold_count >= threshold_num:  # 5번 반복하면서 3번 이상 탐지 성공했을 경우
                        right_servo = True  # 차량을 오른쪽으로 가게 만들어라 flag
                    threshold_left = False
                elif threshold_right:
                    if threshold_count >= threshold_num:  # 5번 반복하면서 3번 이상 탐지 성공했을 경우
            ...
            if state == 1: # Stop 상태
                ...
                elapsedTime = endTime - startTime
                nextRoadWay = taxi.getNextRoadWay()
                if (nextRoadWay == 'turnLeft' and elapsedTime > 1): # If next way is left
            ...
            ######################################################################
            # Show taxi camera with OD and LKAS results
            cv2.imshow('Taxi Camera', heading_image) # Show combined result
