        ...
        while True:
            ...
            for blue_contour in blue_contours:
                ...
                if len(blue_contours) >= 2:
                    ...
                    if state==8 and flag_check_straight:

                        print("왼쪽 사각형 의 y1좌표 : " , y1 , "오른쪽 사각형의 y2좌표 : ", y2)
                        iterate_count += 1
                        if (x1+x2)/2 > 260 and (threshold_right == False):  # 왼쪽으로 5도 이상 치우친 경우
                            if threshold_left:  # 처음이 아닌 경우, 즉 count 중이라면
                                threshold_count += 1
                            else:  # 처음인 경우
                                threshold_left = True
                                threshold_count = 1
                                iterate_count = 1
                        elif (x1+x2)/2 < 220 and (threshold_left == False):  # 오른쪽으로 5도 이상 치우친 경우
                            if threshold_right:
                                threshold_count += 1
                            else:
                                threshold_right = True
                    ...
                    if ((x1 + x2) / 2) >= 140 and ((x1 + x2) / 2) <= 260 and state==8:
                        flag_bluedot = True
                        if (y1+y2)/2 <=270 and (y1+y2)/2 >=260:
                            flag_straight_bluedot = True
            ...
            if state == 1:  # 차 세워
                ...
                if (nextRoadWay == 'turnLeft'):  # If next way is left
                    ...
                    bluetooth.write(result_bytes)
                    print('Control: taxi is now going straight')
                    flag_check_straight = True

                    print("check stragiht 테스트")
                taxi.setState(8)  # Taxi driving in intersection
                startTime = time.time()
                print('Control: taxi now going into intersection')
            elif state == 7:
