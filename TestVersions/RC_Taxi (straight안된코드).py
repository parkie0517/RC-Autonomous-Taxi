    ...
    ############################################################################
        ...
        while True:
            ...
            for blue_contour in blue_contours:
                ...
                if y > 200 and w >= 5 and h >= 5:
                    blue_rects.append(blue_contour)
                    cv2.rectangle(heading_image, (x, y), (x + w, y + h), (0, 255, 0), 5)

            if len(blue_rects) == 2:
                blue_rect1 = cv2.boundingRect(blue_rects[0])
                blue_rect2 = cv2.boundingRect(blue_rects[1])
                x1, y1, w1, h1 = blue_rect1
                x2, y2, w2, h2 = blue_rect2
                nextRoadWay = taxi.getNextRoadWay()
                if nextRoadWay == 'turnLeft' or nextRoadWay == 'turnRight':
                    if ((x1 + x2) / 2) >= 140 and ((x1 + x2) / 2) <= 260 and state==8:
                        flag_bluedot = True
